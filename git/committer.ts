...
import { atHead, commit, ensure, locate, read, staged, stageAll, type Ensured, type Where } from './repo.ts'
...
class Committer {
  ...
  constructor(where: Where, git: GitRunner, quietMs: number, mostMs: number) {
    ...
    this.standing = { committing: false, why: null, pending: false, last: null, trouble: null, root: where.kehikot }
  }

  async start(): Promise<Ensured> {
    const made = await ensure(this.where, this.git)
    if (!made.ok) {
      this.standing = { ...this.standing, committing: false, why: made.why, trouble: made.why }
      return made
    }
 ...
 *
 * Called from the page when it learns which project is open, and from every MCP
 * tool. Both paths converge here so there is one answer to "is this folder
 * versioned yet", and it is idempotent: a second call against a project already
 * running returns what is already running rather than starting a second one.
 *
 * ## Why this is not called from a read
 *
 * `GET /api/histories` deliberately does NOT start anything. A read that created
 * a directory and ran `git init` would mean opening a pane against somebody's
 * project leaves a repository in it they did not ask for. Starting is a
 * `POST`, which the page makes once it knows the project and which an agent
 * makes by using a tool. Both are acts, and both are the kind a person can point
 * at afterwards.
 */
export async function watching(
  ...
  quietMs = QUIET_MS,
  mostMs = MOST_MS,
): Promise<{ ok: boolean; why: string | null; standing: Standing | null; created: boolean }> {
  const found = locate(projectPath)
  if (found.ok === null) return { ok: false, why: null, standing: null, created: false }
  if (found.ok === false) return { ok: false, why: found.why, standing: null, created: false }

  const key = found.where.project
  const already = running.get(key)
  if (already) return { ok: already.standing.committing, why: already.standing.why, standing: already.standing, created: false }

  const one = new Committer(found.where, git, quietMs, mostMs)
  running.set(key, one)
  const made = await one.start()
  if (!made.ok) {
    running.delete(key)
    one.stop()
    return { ok: false, why: made.why, standing: null, created: false }
  }
  return { ok: one.standing.committing, why: one.standing.why, standing: one.standing, created: made.created }
}
 ...
 *
 * Goes through the same committer as the automatic one, so the single-flight
 * guard and the cross-process lock apply to it too. A deliberate commit from a
 * process that is not the committer is refused rather than made — which is the
 * point of having one committer, and would be undone by an exception for the
 * deliberate case.
 */
export async function commitNow(
  ...
  const started = await watching(projectPath, git)
  if (!started.standing) {
    return { ok: false, said: started.why ?? 'There is no project open, so there is no data folder to commit.', sha: null }
  }
  ...
  const found = locate(projectPath)
  if (found.ok !== true) return { ok: false, said: 'That project could not be resolved.', sha: null }
